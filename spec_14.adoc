 ...
 *job*::
 The value of the `job` attribute is a dictionary containing
 additional job metadata.  This metadata may be used for searching and
 filtering of jobs.  Every `value` in the dictionary must be a string.
 The application is free to create keys of any name, however the
 following are reserved for special use:
